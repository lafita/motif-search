...
public class MotifFilter implements Predicate<String> {
	...
	public boolean test(String t) {
		...
		try {
			...
			int oligo = s.getChains().stream().filter(c -> c.isProtein()).
			...
			for (Chain c : s.getChains()) {
				...
				for (int ter = 0; ter < MotifParams.HELIX_LENGTH; ter++) {
					...
					if (ss == null && !dsspRun){
						try{
							dssp.calculate(s, true);
							dsspRun = true;
							ss = (SecStrucState) atomArray[ter + 1].getGroup().getProperty(Group.SEC_STRUC);
						} catch(StructureException e) {
							logger.warn("DSSP fetch for {} failed: {}", t, e);
						}
					}
				...
				logger.debug(String.format("The two terminal helices are at %.0f A apart: %s", distance, t));
